...
def read_plain_images():
    # TODO 读取文件位置
    plainimage_all = loadImageSet('..\data\plainimages\*.jpeg')
    width, height = 640, 480  # 定制尺寸
    resized_images = [cv2.resize(img, (width, height)) for img in plainimage_all]
    # save size information
    img_size = [(height, width)] * len(resized_images)
    # for temp in plainimage_all:
    #     row, col, _ = temp.shape
    #     img_size.append((row, col))
    # TODO 读取文件位置
    np.save("..\data\plainimages.npy", resized_images)
...
def generate_keys(control_length=256*284):
    ...
    # TODO 读取文件位置调整
    keyY = scio.loadmat('..\key.mat')['key'][0]  # Y component encryption key
    # keyY = keyY['key'][0]
    keyCb = scio.loadmat('..\keyCb.mat')['key'][0]  # Cb component encryption key
    # keyCb = keyCb['keyCb'][0]
    keyCr = scio.loadmat('..\keyCr.mat')['key'][0]  # Cr component encryption key
    # keyCr = keyCr['keyCr'][0]
    # keys stream
    s = ksa(keyY)
    r = prga(s, data_lenY)
...
if __name__ == '__main__':
    ...
    encryption_keyY = generate_keys()[0]
    encryption_keyCb = generate_keys()[1]
    encryption_keyCr = generate_keys()[2]
    ...
    # TODO 存储位置自定义
    np.save('acallY.npy', acallY)
    np.save('dcallY.npy', dcallY)
    np.save('acallCb.npy', acallCb)
    np.save('dcallCb.npy', dcallCb)
    np.save('acallCr.npy', acallCr)
    np.save('dcallCr.npy', dcallCr)
